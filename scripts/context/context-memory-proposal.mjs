...
import { parseSessionId, collectSessions } from "../session/m3-ops.mjs"
...
function resolveDirectAiOptions(options = {}, env = process.env) {
  ...
  const rawEndpoint = `${options.endpoint || provider.endpoint || "/chat/completions"}`.trim()
...
export function writeProposal(repoRoot, proposal) {
  ...
  const lines = [
    "---",
    "proposal_version: " + yamlScalar(proposal.proposal_version),
    "id: " + yamlScalar(proposal.id),
    "status: " + yamlScalar(proposal.status),
    "generated_at: " + yamlScalar(proposal.generated_at),
    "source_type: " + yamlScalar(proposal.source_type),
    "source_ref: " + yamlScalar(proposal.source_ref),
    "proposed_document_id: " + yamlScalar(proposal.proposed_document_id),
    "summary: " + yamlScalar(proposal.summary || ""),
    "reviewers:",
  ]
  for (const r of (proposal.reviewers || [])) lines.push("  - " + yamlScalar(r))
  lines.push("---")
  lines.push("")
  lines.push("## Proposed Document")
  lines.push("")
  lines.push("```yaml")
  lines.push("id: " + yamlScalar(fm.id || "unknown"))
  lines.push("kind: " + yamlScalar(fm.kind || "operational-memory"))
  lines.push("crew: " + yamlScalar(fm.crew || "dev"))
  lines.push("agent: " + yamlScalar(fm.agent || "agent"))
  lines.push("capabilities:")
  for (const c of (fm.capabilities || [])) lines.push("  - " + yamlScalar(c))
  lines.push("stability: " + yamlScalar(fm.stability || "draft"))
  lines.push("source_type: " + yamlScalar(fm.source_type || "derived"))
  lines.push("```")
  lines.push("")
...
export async function promoteProposal(repoRoot, proposalId, stability = "curated", options = {}) {
  ...
  // Path safety check for target doc ID
  const docId = fm.proposed_document_id || ""
  if (!docId || /[.]{2}|\\|\0/.test(docId) || !/^[a-z0-9][a-z0-9/_-]*$/i.test(docId)) {
  ...
  for (const [key, val] of Object.entries(docFrontmatter)) {
    if (Array.isArray(val)) {
      if (val.length === 0) {
        ...
        docLines.push(key + ":")
        for (const item of val) docLines.push("  - " + yamlScalar(item))
      }
    } else {
      docLines.push(key + ": " + yamlScalar(val))
    }
  }
  docLines.push("---")
  docLines.push("")
