...
export async function applyProposalFromFile(proposalPath, options = {}) {
  ...
  if (!existsSync(proposalPath)) {
    return { ok: false, error: `Proposal file not found: ${proposalPath}` }
  }

  let proposal
  try {
    proposal = JSON.parse(readFileSync(proposalPath, 'utf-8'))
  } catch {
    return { ok: false, error: `Invalid JSON in proposal file: ${proposalPath}` }
  }
