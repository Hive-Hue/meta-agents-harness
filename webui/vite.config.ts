...
function listRunArtifactsFromSessions(workspaceRoot: string, runtime: string, crew: string, sinceMs: number) {
  ...
  const files: Array<{ path: string; action: "created" | "modified"; size: string; mtimeMs: number }> = [];
...
function listWorkspaceChangedFiles(workspaceRoot: string, sinceMs: number) {
  const files: Array<{ path: string; action: "created" | "modified"; size: string; mtimeMs: number }> = [];
  ...
  while (queue.length > 0 && visited < maxVisited) {
    ...
    for (const entry of entries) {
      ...
      if (stats.mtimeMs < sinceMs - 2000) continue;
      files.push({
        path: path.relative(workspaceRoot, fullPath),
        action: stats.ctimeMs >= sinceMs ? "created" : "modified",
        size: formatBytes(stats.size),
...
function normalizeRuntimeId(runtime: string): string {
  return `${runtime || ""}`.trim().toLowerCase().replace(/^\./, "").replace(/\/+$/, "");
}

function listRuntimeArtifacts(workspaceRoot: string, runtime: string, crew: string, sinceMs: number) {
  ...
  const merged = new Map<string, { path: string; action: "created" | "modified"; size: string; mtimeMs?: number }>();
  for (const item of workspaceArtifacts) merged.set(item.path, item);
  for (const item of sessionArtifacts) merged.set(item.path, item);
...
function runMahCliJson(workspaceRoot: string, args: string[]): Record<string, unknown> {
...
function handleContextSettingsApi(req: import("http").IncomingMessage, res: import("http").ServerResponse) {
  res.setHeader("Content-Type", "application/json");
  const workspaceRoot = resolveWorkspaceRoot(req);
  const workspaceMeta = getWorkspaceMetadata(workspaceRoot);
  if (!workspaceMeta.exists || !workspaceMeta.isDirectory) {
  ...
  if (req.method === "GET") {
    const settings = Object.fromEntries(
      CONTEXT_SETTING_SPECS.map((spec) => {
  ...
  if (req.method === "PUT") {
    ...
    req.on("end", async () => {
      try {
        const bodyRaw = Buffer.concat(chunks).toString("utf-8") || "{}";
        const body = JSON.parse(bodyRaw) as { settings?: Record<string, unknown> };
...
function handleContextVectorActionApi(req: import("http").IncomingMessage, res: import("http").ServerResponse) {
  res.setHeader("Content-Type", "application/json");
  const workspaceRoot = resolveWorkspaceRoot(req);
  const workspaceMeta = getWorkspaceMetadata(workspaceRoot);
  if (!workspaceMeta.exists || !workspaceMeta.isDirectory) {
  ...
  req.on("end", async () => {
    try {
      ...
      const rawEnv = existsSync(envPath) ? readFileSync(envPath, "utf-8") : "";
      const workspaceEnv = parseDotEnvContent(rawEnv);
      const mergedEnv = { ...process.env, ...workspaceEnv } as Record<string, string>;

      const runPython = (scriptName: string, timeout = 180000) => {
...
function handleMissionsApi(req: import("http").IncomingMessage, res: import("http").ServerResponse) {
  ...
  if (req.method === "DELETE" && deleteMatch) {
    try {
      ...
      if (cascade) args.splice(args.length - 1, 0, "--cascade");
...
function handleBootstrapTopologyApi(req: import("http").IncomingMessage, res: import("http").ServerResponse) {
  res.setHeader("Content-Type", "application/json");
  if (req.method !== "POST") {
    res.statusCode = 405;
    res.end(JSON.stringify({ ok: false, error: "method not allowed" }));
  ...
  req.on("end", () => {
    ...
    try {
      ...
      const doc = yaml.load(readFileSync(generatedPath, "utf-8")) as Record<string, unknown>;
      const crewId = `${body.crewId || "dev"}`;
      const crews = Array.isArray(doc?.crews) ? doc.crews as Array<Record<string, unknown>> : [];
      const selectedCrew = crews.find((item) => `${item?.id || ""}` === crewId) || crews[0];
      const topology = (selectedCrew?.topology || {}) as Record<string, unknown>;
      const leads = (topology.leads || {}) as Record<string, string>;
      const workers = (topology.workers || {}) as Record<string, string[]>;
...
function handleBootstrapWriteApi(req: import("http").IncomingMessage, res: import("http").ServerResponse) {
  res.setHeader("Content-Type", "application/json");
  if (req.method !== "POST") {
    res.statusCode = 405;
    res.end(JSON.stringify({ ok: false, error: "method not allowed" }));
