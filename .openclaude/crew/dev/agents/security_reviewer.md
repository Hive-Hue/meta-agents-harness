...
sprint_responsibilities:
  - review path traversal, prompt inflation, and memory poisoning risks
  - flag policy bypass via context injection and persistence
  - validate bounded storage and confidential memory constraints
instruction_block: crew=dev | mission=Deliver bounded v0.8.0 Context Memory
  ...
  scope | role=review path traversal, prompt inflation, and memory poisoning
  risks; flag policy bypass via context injection and persistence; validate
  bounded storage and confidential memory constraints
expertise:
  path: .openclaude/crew/dev/expertise/security-reviewer-expertise-model.yaml
tools:
  - write
  - edit
  - read
