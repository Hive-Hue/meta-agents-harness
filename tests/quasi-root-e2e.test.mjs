...
test("domain enforcement also applies to leads for out-of-domain bash deletes", () => {
  const script = `
import { mkdtempSync, mkdirSync, readFileSync, rmSync, writeFileSync } from "node:fs";
import os from "node:os";
import path from "node:path";
...
const repoRoot = ${JSON.stringify(repoRoot)};
const tempDir = mkdtempSync(path.join(os.tmpdir(), "mah-domain-lead-"));
const extPath = path.join(repoRoot, "extensions", "multi-team.ts");
const source = readFileSync(extPath, "utf-8");
const compiled = ts.transpileModule(source, {
  compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ES2022 }
...
process.env.MAH_RUNTIME = "pi";
process.env.MAH_ACTIVE_CREW = "dev";
process.env.MAH_MULTI_ROLE = "lead";
process.env.MAH_MULTI_AGENT = "engineering-lead";
process.env.MAH_MULTI_TEAM = "Engineering";
delete process.env.PI_MULTI_HEADLESS;
...
let aborted = false;
const guard = await handlers.get("tool_call")(
  { toolName: "bash", input: { command: "rm /home/ec2-user/ngrok.log" } },
  { ...baseCtx, abort() { aborted = true; } }
);

console.log(JSON.stringify({ guard, aborted }));
rmSync(tempDir, { recursive: true, force: true });
`;
  ...
  assert.equal(payload.guard.block, true);
  assert.equal(payload.aborted, true);
  assert.match(payload.guard.reason, /outside read scope|access denied|guardrail/i);
});
