...
test("kilo explain run resolves to direct cli with injected crew context", () => {
  ...
  assert.match(kiloConfig.agent.orchestrator.prompt, /Mission: Deliver bounded v0\.7\.0 Expertise Engine evolution/)
  assert.match(kiloConfig.agent.orchestrator.prompt, /Sprint: v0\.7\.0-expertise-engine/)
  assert.match(kiloConfig.agent.orchestrator.prompt, /Target release: v0\.7\.0/)
...
test("opencode explain run resolves to direct cli without wrapper plan", () => {
  ...
  assert.deepEqual(payload.execArgs, [])
})
...
test("hermes explain run resolves to hermes chat with MAH bootstrap env", () => {
  ...
  assert.deepEqual(payload.execArgs, ["chat", "-m", "minimax/minimax-m2.7"])
  assert.equal(payload.env?.MAH_ACTIVE_CREW, "dev")
