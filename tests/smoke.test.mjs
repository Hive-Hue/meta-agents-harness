...
test("bootstrap script creates minimal meta-agents.yaml in non-interactive mode", () => {
  ...
  try {
    ...
    assert.equal(parsed.domain_profiles?.bootstrap_generation, undefined)
    assert.equal(parsed.domain_profiles?.runtime_assets_sync, undefined)
    assert.equal(parsed.domain_profiles?.docs_authoring, undefined)
