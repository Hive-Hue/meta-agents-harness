			...
			if (isToolCallEventType("bash", event)) {
				...
				if (!violationReason) {
					for (const rop of rules.readOnlyPaths) {
						if (!command.includes(rop)) continue;
						// Heuristic: check if command might modify a read-only path
						// Redirects, sed -i, rm, mv to, etc.
						const editingKeywords = ["rm", "mv", "sed"];
						const usesEditingKeyword = editingKeywords.some((keyword) => new RegExp(`\\b${keyword}\\b`).test(command));
						const usesRedirection = />>?|>\|/.test(command);
						if (usesEditingKeyword || usesRedirection) {
