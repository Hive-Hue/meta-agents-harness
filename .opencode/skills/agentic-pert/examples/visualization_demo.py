...
def main():
    ...
    output_dir = os.path.expanduser("~/.openclaw/workspace/skills/agentic-pert/examples/output")
    os.makedirs(output_dir, exist_ok=True)
    
    # Save all formats
    with open(f"{output_dir}/plan.mmd", "w") as f:
        f.write(mermaid)
    
    with open(f"{output_dir}/plan-d3.json", "w") as f:
        json.dump(d3_data, f, indent=2)
    
    with open(f"{output_dir}/plan-flow.json", "w") as f:
        json.dump(react_flow, f, indent=2)
    
    with open(f"{output_dir}/plan.dot", "w") as f:
        f.write(dot)
