...
sprint_responsibilities:
  - document bootstrap examples
  - document AI-assisted mode
  - document defaults and required fields
...
instruction_block: crew=bootstrap-config | mission=Design and implement an
  ...
  needs; v0.5.0+ onboarding wizard scope | role=document bootstrap examples;
  document AI-assisted mode; document defaults and required fields; document
  CI/non-interactive usage
expertise:
  path: .hermes/crew/bootstrap-config/expertise/docs-reviewer-expertise-model.yaml
tools:
  - write
  - edit
  - read
