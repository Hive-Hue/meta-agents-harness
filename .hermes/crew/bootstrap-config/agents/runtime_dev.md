...
sprint_responsibilities:
  - generate meta-agents.yaml
  - integrate with validate:config
  - support AI-assisted config completion contract
  - add safe merge/overwrite behavior
instruction_block: crew=bootstrap-config | mission=Design and implement an
  ...
  needs; v0.5.0+ onboarding wizard scope | role=generate meta-agents.yaml;
  integrate with validate:config; support AI-assisted config completion
  contract; add safe merge/overwrite behavior
expertise:
  path: .hermes/crew/bootstrap-config/expertise/runtime-dev-expertise-model.yaml
tools:
  - write
  - edit
  - read
...
skills:
  ...
  - path: .hermes/skills/bootstrap-config-architect/SKILL.md
    use-when: Use when relevant to current task.
domain:
  - path: .
    read: true
