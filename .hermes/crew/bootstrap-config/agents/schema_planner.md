...
sprint_responsibilities:
  - identify minimally required fields
  - define generated config skeleton
  - map required fields to validation rules
instruction_block: crew=bootstrap-config | mission=Design and implement an
  ...
  needs; v0.5.0+ onboarding wizard scope | role=identify minimally required
  fields; define generated config skeleton; map required fields to validation
  rules
expertise:
  path: .hermes/crew/bootstrap-config/expertise/schema-planner-expertise-model.yaml
tools:
  - write
  - edit
  - read
